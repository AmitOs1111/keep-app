...
function query(entityType, filterBy, delay = 400) {
  ...
  if (filterBy) {
    const { type, isTrash, isArchive, label, search, labels, category, color } =
      filterBy
    if (search) {
      const regExp = new RegExp(search, 'i')

      const titles = entities.filter((note) => regExp.test(note.info.title))
      ...
      entities = _connectNotes(txts, todos, titles)
    }
    ...
    if (type) {
      ...
      entities = entities.filter((note) => regExp.test(note.info.type))
    }
  }
...
function put(entityType, updatedEntity) {
  if (
    typeof updatedEntity === 'object' &&
    !Array.isArray(updatedEntity) &&
    updatedEntity !== null
  ) {
  ...
  if (Array.isArray(updatedEntity)) {
    ...
    return query(entityType).then((fullNotes) => {
      const restNotes = fullNotes.filter(
        (entity) => !updatedEntity.find((note) => note._id === entity._id)
      )

      _save(entityType, [...updatedEntity, ...restNotes])
      return updatedEntity
    })
  }
}
...
function _connectNotes(txts, todos, titles) {
  const res = []
  const notes = [txts, todos, titles]
